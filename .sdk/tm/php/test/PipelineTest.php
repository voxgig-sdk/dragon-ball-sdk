...
{
    private static function utility(): DragonBallUtility
    {
        return new DragonBallUtility();
    }
    ...
    private static function ctx(array $over = []): DragonBallContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new DragonBallContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new DragonBallOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof DragonBallError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new DragonBallOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = DragonBallMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new DragonBallOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = DragonBallMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new DragonBallOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = DragonBallMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DragonBallSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = DragonBallMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new DragonBallResponse([]),
            'result' => new DragonBallResult([]),
        ]);
        [, $err] = DragonBallMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new DragonBallSpec([]),
            'response' => null,
            'result' => new DragonBallResult([]),
        ]);
        [, $err] = DragonBallMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new DragonBallSpec(['step' => 's']),
            'response' => new DragonBallResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new DragonBallResult([]),
        ]);
        [, $err] = DragonBallMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new DragonBallSpec(['step' => 's']),
            'response' => new DragonBallResponse(self::resp(200, ['v' => 1])),
            'result' => new DragonBallResult([]),
        ]);
        [, $err] = DragonBallMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new DragonBallSpec(['step' => 's']),
            'response' => new DragonBallResponse(self::resp(200, ['v' => 2])),
            'result' => new DragonBallResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        DragonBallMakeResponse::call($ctx);
    ...
    {
        $preset = new DragonBallResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new DragonBallSpec([]),
            'response' => new DragonBallResponse([]),
            'result' => new DragonBallResult([]),
        ...
        [$got, $err] = DragonBallMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new DragonBallResult([])]);
        [, $err] = DragonBallMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new DragonBallSpec([]), 'result' => null]);
        [, $err] = DragonBallMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DragonBallSpec(['step' => 's']),
            'result' => new DragonBallResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new DragonBallOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DragonBallMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new DragonBallSpec(['step' => 's']),
            'result' => new DragonBallResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new DragonBallOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = DragonBallMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new DragonBallResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new DragonBallSpec([]),
            'result' => new DragonBallResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = DragonBallMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DragonBallMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new DragonBallError('boom', 'boom');
        $utility->fetcher = function (DragonBallContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DragonBallSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = DragonBallMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (DragonBallContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = DragonBallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(DragonBallResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        DragonBallMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (DragonBallContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new DragonBallSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = DragonBallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new DragonBallResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new DragonBallSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = DragonBallMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = DragonBallMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = DragonBallMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new DragonBallResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, DragonBallDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new DragonBallResult(['ok' => false])]);
        $this->expectException(DragonBallError::class);
        DragonBallDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new DragonBallResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', DragonBallMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        DragonBallMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new DragonBallBaseFeature();
        $b = new DragonBallBaseFeature();
        DragonBallFeatureAdd::call($ctx, $a);
        DragonBallFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): DragonBallBaseFeature
    {
        $f = new DragonBallBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        DragonBallFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): DragonBallContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new DragonBallSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = DragonBallPrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        DragonBallPrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new DragonBallResponse(['headers' => null]),
            'result' => new DragonBallResult([]),
        ]);
        DragonBallResultHeaders::call($ctx);
