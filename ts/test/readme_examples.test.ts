...
//     (\`new DragonBallSDK(...)\` / \`DragonBallSDK.test(...)\` become
//     \`DragonBallSDK.test({ entity: ... })\`), transpiled to JS, and run
...
const TEST_SEED = {"entity":{"character":{"test01":{"id":"test01"}},"planet":{"test01":{"id":"test01"}},"transformation":{"test01":{"id":"test01"}}}}
...
// rewrite real client construction (\`new DragonBallSDK(...)\` and any
// \`DragonBallSDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+DragonBallSDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/DragonBallSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
