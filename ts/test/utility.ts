...
function envOverride(m: Record<string, any>) {
  if (
    'TRUE' === process.env.DRAGONBALL_TEST_LIVE ||
    'TRUE' === process.env.DRAGONBALL_TEST_OVERRIDE
  ) {
  ...
  m.DRAGONBALL_TEST_EXPLAIN = process.env.DRAGONBALL_TEST_EXPLAIN || m.DRAGONBALL_TEST_EXPLAIN
