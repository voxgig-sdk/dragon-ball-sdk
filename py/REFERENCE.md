# DragonBall Python SDK Reference

Complete API reference for the DragonBall Python SDK.
...
#### `DragonBallSDK.test(testopts=None, sdkopts=None)`
...
#### `Transformation(data=None)`

Create a new `TransformationEntity` instance. Pass `None` for no initial data.
...
```python
results, err = client.Transformation().list({})
```
...
```python
result, err = client.Transformation().load({"id": "transformation_id"})
```
...
Create a new `TransformationEntity` instance with the same options.
