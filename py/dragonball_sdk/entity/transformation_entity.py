...
from dragonball_sdk.dragonball_types import (
    Transformation,
    TransformationLoadMatch,
    TransformationListMatch,
)
...
class TransformationEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return TransformationEntity(self._client, opts)
    ...
    def data_get(self) -> Transformation:
    ...
    def match_get(self) -> Transformation:
    ...
    def load(self, reqmatch=None, ctrl=None) -> Transformation:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.Transformation().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[Transformation]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.Transformation().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
        ctx = utility.make_context({
            "opname": "list",
