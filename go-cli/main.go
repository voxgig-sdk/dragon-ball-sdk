// Command DragonBall-cli is a CLI + REPL for the DragonBall SDK,
...
const prompt = "dragon-ball"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "character planet transformation"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      DRAGON_BALL_APIKEY (api key), DRAGON_BALL_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: DRAGON_BALL_APIKEY carries the API key and
	// DRAGON_BALL_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("DRAGON_BALL_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("DRAGON_BALL_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewDragonBallSDK(opts)
