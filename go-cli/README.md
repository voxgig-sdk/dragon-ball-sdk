...
boru-driven command-line client **and** interactive REPL for the DragonBall
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dragon-ball-cli)
make build

# 2. See usage (words, entities, env vars)
./dragon-ball-cli --help

# 3. Provide credentials once, via the environment
export DRAGON_BALL_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./dragon-ball-cli list character
./dragon-ball-cli load 1 character            # {id:1} shorthand
./dragon-ball-cli load '{id:1}' character       # explicit match map
./dragon-ball-cli list planet

# 5. Override the API base URL for a single call
DRAGON_BALL_BASE=https://api.example.com ./dragon-ball-cli list character

# 6. No arguments -> interactive REPL
./dragon-ball-cli
dragon-ball> list character
dragon-ball> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dragon-ball-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/dragon-ball-cli list character
   ```
...
```sh
./dragon-ball-cli load 1 character          # scalar shorthand for {id:1}
./dragon-ball-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `dragon-ball>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/dragon-ball-cli`. |
...
(`./dragon-ball-cli <expr>`) and interactively (the REPL), and expressions compose the
