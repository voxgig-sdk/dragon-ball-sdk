...
[MCP](https://modelcontextprotocol.io) server exposing the DragonBall SDK as
two agent tools — `dragon-ball_list` and `dragon-ball_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/dragon-ball-mcp)
make build

# 2. Provide credentials via the environment
export DRAGON_BALL_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user dragon-ball \
  -- /absolute/path/to/dragon-ball-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./dragon-ball-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// dragon-ball_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// dragon-ball_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/dragon-ball-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user dragon-ball \
     -- "$PWD"/dist/*/dragon-ball-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `dragon-ball_list` and `dragon-ball_load` tools now appear
   in new sessions. Ask the agent to *"list character using dragon-ball"*
   and it calls `dragon-ball_list` with `{"entity":"character"}`.
...
```sh
export DRAGON_BALL_APIKEY=sk_live_xxx            # API key
export DRAGON_BALL_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `dragon-ball_list` tool
...
### Call the `dragon-ball_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `dragon-ball_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `dragon-ball_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `DRAGON_BALL_APIKEY` | API key sent with every request. |
| `DRAGON_BALL_BASE` | Optional override of the API base URL. |

### Entities

The 3 entities valid as the `entity` argument:

character | planet | transformation

### Smoke test via HTTP (raw JSON-RPC)

```sh
./dragon-ball-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"dragon-ball_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
