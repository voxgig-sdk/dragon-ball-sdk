...
The Lua SDK for the DragonBall API — an entity-oriented client using Lua conventions.
...
Create an instance: `local transformation = client:Transformation(nil)`
...
```lua
local transformation, err = client:Transformation():load({ id = "transformation_id" })
```

#### Example: List

```lua
local transformations, err = client:Transformation():list()
```
...
```
lua/
├── dragon-ball_sdk.lua    -- Main SDK module
...
The main module (`dragon-ball_sdk`) exports the SDK constructor
