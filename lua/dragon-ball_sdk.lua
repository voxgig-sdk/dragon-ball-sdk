...
local DragonBallSDK = {}
DragonBallSDK.__index = DragonBallSDK
...
DragonBallSDK._make_feature = _make_feature


function DragonBallSDK.new(options)
  local self = setmetatable({}, DragonBallSDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function DragonBallSDK:options_map()
...
function DragonBallSDK:get_utility()
  return Utility.copy(self._utility)
end


function DragonBallSDK:get_root_ctx()
  return self._rootctx
end


function DragonBallSDK:prepare(fetchargs)
...
function DragonBallSDK:direct(fetchargs)
...
-- by passing it.
function DragonBallSDK:_raw_request(fetchargs)
  local utility = self._utility
...
function DragonBallSDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "DragonBallSDK: graphql: " .. msg
    res.graphql = errors
  end
...
function DragonBallSDK:Transformation(data)
  ...
  if data == nil then
    if self._transformation == nil then
      self._transformation = EntityMod.new(self, nil)
    end
...
function DragonBallSDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = DragonBallSDK.new(sdkopts)
  sdk.mode = "test"
