...
describe("TransformationDirect", function()
  it("should direct-list-transformation", function()
    local setup = transformation_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-transformation", setup.live and "live" or "unit")
  ...
  it("should direct-load-transformation", function()
    local setup = transformation_direct_setup({ id = "direct01" })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-load-transformation", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
...
function transformation_direct_setup(mockres)
