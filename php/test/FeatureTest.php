...
class FtCtrl extends DragonBallControl
{
...
class FtClient
{
    ...
    private DragonBallUtility $utility;

    public function __construct(DragonBallUtility $utility, array $options)
    {
    ...
    public function get_utility(): DragonBallUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (DragonBallContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public DragonBallContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (DragonBallContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = DragonBallConfig::make_feature($name);
    ...
    private static function buildUrl(DragonBallSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof DragonBallError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new DragonBallResponse($fetched);
        }
    ...
    private function fail(DragonBallContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (DragonBallContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
