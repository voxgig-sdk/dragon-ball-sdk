...
The PHP SDK for the DragonBall API — an entity-oriented client using PHP conventions.
...
print_r(array_map(fn($item) => $item->data_get(), $planet));
```
...
| `originPlanet` | `array` | Planet where the character originated |
...
```
php/
├── dragonball_sdk.php          -- Main SDK class
...
The main class (`dragonball_sdk.php`) exports the SDK class
