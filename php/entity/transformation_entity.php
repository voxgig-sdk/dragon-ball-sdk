...
// DragonBall SDK Transformation entity
...
class TransformationEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new TransformationEntity($this->_client, $opts);
    }

    /**
     * @param Transformation|array $args Transformation data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = DragonBallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return Transformation|array The current Transformation data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Transformation fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = DragonBallHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of Transformation fields).
     */
    ...
    /**
     * Load a single Transformation.
     *
     * @param TransformationLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed TransformationLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Transformation|array The loaded Transformation as an assoc-array at the
     *   SDK boundary; throws DragonBallError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = DragonBallHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
    ...
    /**
     * List Transformation items matching the given filter.
     *
     * @param TransformationListMatch|array|null $reqmatch Match filter (any subset
     *   of Transformation fields) as an assoc-array; TransformationListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Transformation[]|array A list of Transformation items as assoc-arrays at
     *   the SDK boundary; throws DragonBallError on failure (item-5 convention).
     */
