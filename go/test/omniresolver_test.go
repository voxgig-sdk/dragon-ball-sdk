...
import (
	...
	omni "github.com/voxgig-sdk/dragon-ball-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.DragonBallError); is {
				out := map[string]any{
					"name":    "DragonBallError",
...
func omniCtx(arg any, client *sdk.DragonBallSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.DragonBallSDK); is {
			client = live
