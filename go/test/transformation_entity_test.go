...
func TestTransformationEntity(t *testing.T) {
	...
	t.Run("stream", func(t *testing.T) {
		...
		var seen []any
		for item := range base.Transformation(nil).Stream("list", nil, nil) {
		...
		if hasStreaming {
			...
			var got []any
			for item := range streamSdk.Transformation(nil).Stream("list", nil, nil) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		transformationRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.transformation")))
		...
		transformationRef01DataDt0LoadResult := core.ToMapAny(entityData(transformationRef01DataDt0Loaded))
...
func transformationBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("DRAGON_BALL_TEST_TRANSFORMATION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["DRAGON_BALL_TEST_TRANSFORMATION_ENTID"])
