# DragonBall Golang SDK



The Golang SDK for the DragonBall API — an entity-oriented client using standard Go conventions. No generics required; data flows as `map[string]any`.
...
## Install
```bash
go get github.com/voxgig-sdk/dragon-ball-sdk/go@latest
```

The Go module proxy resolves the version from the `go/vX.Y.Z` GitHub
release tag — see [Releases](https://github.com/voxgig-sdk/dragon-ball-sdk/releases) for the available versions.
...
```bash
go mod edit -replace github.com/voxgig-sdk/dragon-ball-sdk/go=../dragon-ball-sdk/go
```
...
```go
func TestSDK(testopts map[string]any, sdkopts map[string]any) *DragonBallSDK
```

Creates a test-mode client with mock transport. Both arguments may be `nil`.

### DragonBallSDK methods
...
| `Character` | `(data map[string]any) DragonBallEntity` | Create a Character entity instance. |
| `Planet` | `(data map[string]any) DragonBallEntity` | Create a Planet entity instance. |
| `Transformation` | `(data map[string]any) DragonBallEntity` | Create a Transformation entity instance. |

### Entity interface (DragonBallEntity)

All entities implement the `DragonBallEntity` interface.
...
| `originPlanet` | `map[string]any` | Planet where the character originated |
...
| `isDestroyed` | `bool` | Whether the planet has been destroyed |
...
Create an instance: `transformation := client.Transformation(nil)`
...
| `id` | `int` | Unique identifier for the transformation |
...
```go
transformation, err := client.Transformation(nil).Load(map[string]any{"id": 1}, nil)
if err != nil {
    panic(err)
}
fmt.Println(transformation) // the loaded record
```

#### Example: List

```go
transformations, err := client.Transformation(nil).List(nil, nil)
if err != nil {
    panic(err)
}
fmt.Println(transformations) // the array of records
```
...
```
github.com/voxgig-sdk/dragon-ball-sdk/go/
├── dragon-ball.go        # Root package — type aliases and constructors
...
The root package (`github.com/voxgig-sdk/dragon-ball-sdk/go`) re-exports everything needed
