...
require_relative 'DragonBall_types'
...
class DragonBallSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = DragonBallHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = DragonBallHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, DragonBallFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    DragonBallUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = DragonBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = DragonBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = DragonBallHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => DragonBallError.new(
        "#{op}_allow",
        "DragonBallSDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = DragonBallHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = DragonBallError.new(
        "graphql_error", "DragonBallSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
