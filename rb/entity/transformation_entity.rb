...
class TransformationEntity
  ...
  def make
    opts = @_entopts.dup
    TransformationEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = DragonBallHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [Transformation, Hash] the current Transformation data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = DragonBallHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of Transformation fields)
  def match_get
  ...
  #
  # @param reqmatch [TransformationLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Transformation.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Transformation, Hash] the loaded Transformation; raises DragonBallError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = DragonBallHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  #
  # @param reqmatch [TransformationListMatch, Hash, nil] match filter (any subset of
  #   Transformation fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Transformation>, Array] the matching Transformation items; raises DragonBallError on failure
