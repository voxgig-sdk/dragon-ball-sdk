...
module DragonBallUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = DragonBallResponse.new({})
    ...
    if fetch_err
      ...
      response = DragonBallResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = DragonBallResponse.new(fetched)
    else
